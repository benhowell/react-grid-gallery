...
var Gallery = function (_Component) {
    ...
    _createClass(Gallery, [{
        ...
        key: 'onClickImage',
        value: function onClickImage(index, image, event) {
            if (this.state.currentImage === this.props.images.length - 1) return;
            this.gotoNext();
        }
    }, {
        key: 'onSelectImage',
        value: function onSelectImage(index, image, event) {
            event.preventDefault();
            if (this.props.onSelectImage) this.props.onSelectImage(index, this.state.images[index], event);
        }
    }, {
